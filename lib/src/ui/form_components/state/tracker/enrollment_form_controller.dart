...
class D2TrackerEnrollmentFormController extends D2FormController
  ...
  D2TrackerEnrollmentFormController({
    ...
    if (!editMode) {
      if (orgUnit != null) {
        ...
        addListener(() {
          if (formValues['orgUnit'] != null) {
            if (orgUnit != formValues['orgUnit']) {
              orgUnit = formValues['orgUnit'];
              getAutoGeneratedValues();
            }
          }
        });
  ...
  disableAutoGeneratedFields() {
    List<D2ProgramTrackedEntityAttribute> autogeneratedAttributes = program
        ...
        .where((D2ProgramTrackedEntityAttribute pAttribute) =>
            ...
            pAttribute.renderType != "BAR_CODE")
        .toList();
  ...
  getAutoGeneratedValues() {
    String? orgUnitId = orgUnit ?? formValues['orgUnit'];
    if (orgUnitId != null) {
      List<D2ProgramTrackedEntityAttribute> autogeneratedAttributes = program
          .programTrackedEntityAttributes
          .where((D2ProgramTrackedEntityAttribute pAttribute) =>
  ...
  Future<D2Enrollment> create() async {
    ...
    D2OrgUnit? orgUnit =
        D2OrgUnitRepository(db).getByUid(this.orgUnit ?? formValues['orgUnit']);
    if (orgUnit == null) {
